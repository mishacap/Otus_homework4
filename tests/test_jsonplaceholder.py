...
@pytest.mark.parametrize(
    ["data", "query"],
    [read_lines_from_csv_update(), read_ids_from_csv_update()],
...
def test_create_post(jsonplaceholder_api_client, data, query):
    response = jsonplaceholder_api_client.update_post(data=data)
    json_response = response.json()
    assert response.status_code == 200
    assert response.json()
    validate(instance=json_response, schema=create_and_update_post_schema)
